...
namespace Xamarin.Plugin.FirebaseRemoteConfig
{
    ...
    {
        private static readonly Lazy<IFirebaseRemoteConfig> _filePickerService =
            new Lazy<IFirebaseRemoteConfig>(CreateFirebaseRemoteConfig, LazyThreadSafetyMode.PublicationOnly);
        ...
        private static T Instance<T>(Lazy<T> lazy) where T : class
        {
            var ret = lazy.Value;
            if (ret == null)
            {
